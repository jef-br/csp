//! Compile-time tuning constants — parked here alongside the detection/processor logic that reads
//! them. `src/core/config.rs` keeps only `JPEG_QUALITY`, the one constant the essential
//! load → preproc → save pipeline still needs. Re-admit sections of this file as their owning
//! module comes back from `to review/`.
//!
